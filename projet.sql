...
BEGIN
  ...
  END IF;
  IF UPDATING THEN
    Update MV_VENTE_MEDICAMENTS set SECTOR_ID = :NEW.SECTOR_ID, TIME_ID = :NEW.TIME_ID, ID_MEDICAMENT = :NEW.ID_MEDICAMENT, CA = :NEW.CA, MARGE=:NEW.MARGE where SECTOR_ID=:OLD.SECTOR_ID and TIME_ID=:OLD.TIME_ID and ID_MEDICAMENT=:OLD.ID_MEDICAMENT and CA=:OLD.CA and MARGE=:OLD.MARGE;
    DBMS_OUTPUT.PUT_LINE('Modification');
  END IF;
  IF DELETING THEN
    Delete from MV_VENTE_MEDICAMENTS where SECTOR_ID=:OLD.SECTOR_ID and TIME_ID=:OLD.TIME_ID and ID_MEDICAMENT=:OLD.ID_MEDICAMENT and CA=:OLD.CA and MARGE=:OLD.MARGE;
